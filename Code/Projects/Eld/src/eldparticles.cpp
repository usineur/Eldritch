...
/*static*/ EldParticles::TParamCache EldParticles::sm_ParamCache;

EldParticles::EldParticles()
:	m_Params()
,	m_SystemLocation()
,	m_SystemOrientation()
...
EldParticles::~EldParticles()
{
	IRenderer* const pRenderer = EldFramework::GetInstance()->GetRenderer();
...
EldParticles::SParticle::SParticle()
//:	m_Lifetime( 0.0f )
:	m_SpawnTime( 0.0f )
,	m_ExpireTime( 0.0f )
,	m_Size( 0.0f )
...
void EldParticles::GetCachedConfig( const SimpleString& DefinitionName )
{
...
/*static*/ void EldParticles::FlushConfigCache()
{
	sm_ParamCache.Clear();
}

void EldParticles::InitializeFromDefinition( const SimpleString& DefinitionName )
{
...
void EldParticles::Tick( const float DeltaTime )
{
	//PROFILE_FUNCTION;
...
void EldParticles::SpawnParticles( uint NumParticlesToSpawn )
{
	XTRACE_FUNCTION;
...
Vector EldParticles::GetParticleSpawnLocationOffset() const
{
...
void EldParticles::TickParticles( const float DeltaTime )
{
	if( IsUnsorted() )
	{
	...
	{
		FOR_EACH_LIST_NOINCR( ParticleIter, m_ParticlesList, EldParticles::SParticle )
		{
...
bool EldParticles::TickParticle( SParticle& Particle, float DeltaTime )
{
	const float CurrentTime = GetTime();
	...
	if( m_Params.m_Collision )
	{
		...
		if( pWorld->Trace( TraceSegment, Info ) )
		{
			...
			Vector NormalVelocity = Particle.m_Velocity.ProjectionOnto( Info.m_Out_Plane.m_Normal );
			Vector TangentialVelocity = Info.m_Out_Plane.ProjectVector( Particle.m_Velocity );
...
void EldParticles::CreateMesh()
{
	...
	else
	{
		...
		uint VertexSignature = VD_POSITIONS | VD_UVS | VD_FLOATCOLORS | VD_NORMALS;
...
inline uint EldParticles::GetNumParticles() const
{
	return IsUnsorted() ? m_ParticlesArray.Size() : m_ParticlesList.Size();
}

void EldParticles::InitVertexBuffers( IVertexBuffer* pVertexBuffer )
{
	//PROFILE_FUNCTION;
...
void EldParticles::UpdateMesh()
{
	...
	View* const pView	= EldFramework::GetInstance()->GetMainView();
	DEVASSERT( pView );
	...
	if( IsUnsorted() )
	{
		...
		{
			EldParticles::SParticle& Particle = m_ParticlesArray[ ParticleIndex ];
	...
	else
	{
		...
		FOR_EACH_LIST( ParticleIter, m_ParticlesList, EldParticles::SParticle )
		{
			EldParticles::SParticle& Particle = ParticleIter.GetValue();
			AddParticleToMesh( ParticleIndex, Particle, MeshParams );
			++ParticleIndex;
		}
	}
...
inline void EldParticles::AddParticleToMesh( const uint Index, const SParticle& Particle, const SParticleMeshParams& MeshParams )
{
	uint j = Index * 4;
	uint k = j + 1;
	uint l = k + 1;
...
float EldParticles::GetViewDistanceSq( const Vector& ParticleLocation )
{
	View* const pView = EldFramework::GetInstance()->GetMainView();
	return ( pView->m_Location - ParticleLocation ).LengthSquared();
}

/*static*/ void EldParticles::DeviceResetCallback( void* pVoid, IVertexBuffer* pBuffer )
{
	EldParticles* const pParticleSystem = static_cast<EldParticles*>( pVoid );
	pParticleSystem->InitVertexBuffers( pBuffer );
}

inline bool EldParticles::IsUnsorted() const
{
	return m_Params.m_AdditiveDraw || m_Params.m_AlphaTestDraw;
}

inline float EldParticles::GetTime() const
{
	return WBWorld::GetInstance()->GetTime();
}

inline bool EldParticles::SystemIsFinite() const
{
	return m_Params.m_SystemLifetime > 0.0f || m_Params.m_SpawnRate == 0.0f;
}

inline bool EldParticles::IsExpired() const
{
	return SystemIsFinite() && GetTime() >= m_ExpireTime;
}

bool EldParticles::IsFinished() const
{
