...
static const int	kMaxExits		= 1 + ( EldWorldGen::ERE_North | EldWorldGen::ERE_South | EldWorldGen::ERE_East | EldWorldGen::ERE_West | EldWorldGen::ERE_Up | EldWorldGen::ERE_Down );
static const int	kMaxTransforms	= 1 + ( EldWorldGen::ERT_MirrorX | EldWorldGen::ERT_MirrorY | EldWorldGen::ERT_Rotate90 );

EldWorldGen::EldWorldGen()
:	m_World( NULL )
,	m_RoomDefs()
,	m_FeatureRoomDefs()
,	m_Themes()
...
void EldWorldGen::Initialize( const SimpleString& WorldGenDefinitionName )
{
	m_World = EldFramework::GetInstance()->GetWorld();
	ASSERT( m_World );
...
void EldWorldGen::InitializeFeatureRoomDef( const SimpleString& FeatureRoomDefName )
{
...
void EldWorldGen::InitializeRoomDefs( Array<uint>* pRoomDefs, const uint NumRoomDefs, TRoomMap* pRoomMap, const SimpleString& DefinitionName )
{
...
void EldWorldGen::ComputeExitTransforms()
{
	PROFILE_FUNCTION;
...
int EldWorldGen::GetPrecomputedTransformedExits( const int Exits, const int Transform )
{
	return m_ExitTransforms[ Exits + Transform * kMaxExits ];
}

int EldWorldGen::GetTransformedExits( const int Exits, const int Transform )
{
...
int EldWorldGen::GetUntransformedExits( const int Exits, const int Transform )
{
	int RetVal = Exits & ( ERE_Up | ERE_Down );

	if( Transform & ERT_Rotate90 )
	{
		if( Exits & ERE_North )	{ RetVal |= ERE_East; }
		if( Exits & ERE_South )	{ RetVal |= ERE_West; }
		if( Exits & ERE_West )	{ RetVal |= ERE_North; }
		if( Exits & ERE_East )	{ RetVal |= ERE_South; }
	}
	else
	{
		RetVal |= Exits & ( ERE_North | ERE_South | ERE_East | ERE_West );
	}

	if( Transform & ERT_MirrorY )
	{
		const int Temp = RetVal;
		RetVal &= ( ERE_Up | ERE_Down | ERE_East | ERE_West );

		if( Temp & ERE_North )	{ RetVal |= ERE_South; }
		if( Temp & ERE_South )	{ RetVal |= ERE_North; }
	}

	if( Transform & ERT_MirrorX )
	{
		const int Temp = RetVal;
		RetVal &= ( ERE_Up | ERE_Down | ERE_North | ERE_South );

		if( Temp & ERE_East )	{ RetVal |= ERE_West; }
		if( Temp & ERE_West )	{ RetVal |= ERE_East; }
	}

	return RetVal;
}

inline void EldWorldGen::AddExit( ERoomExits& Exits, const ERoomExits NewExit )
{
...
HashedString EldWorldGen::GetThemeFromFilename( const SimpleString& RoomFilename )
{
...
void EldWorldGen::GetExitsFromFilename( const SimpleString& RoomFilename, ERoomExits& Exits )
{
...
EldWorldGen::ERoomExits EldWorldGen::GetExit( const HashedString& ExitName ) const
{
...
EldWorldGen::ERoomExits EldWorldGen::GetExit( const char ExitChar ) const
{
...
SimpleString EldWorldGen::GetExitString( const ERoomExits& Exits ) const
{
	if( Exits == ERE_None )
	{
		return "x";
	...
	SimpleString RetVal;
	if( Exits & ERE_North )	{ RetVal += "n"; }
	if( Exits & ERE_South )	{ RetVal += "s"; }
	if( Exits & ERE_East )	{ RetVal += "e"; }
	if( Exits & ERE_West )	{ RetVal += "w"; }
	if( Exits & ERE_Up )	{ RetVal += "u"; }
	if( Exits & ERE_Down )	{ RetVal += "d"; }
	return RetVal;
}

inline int EldWorldGen::GetRoomIndex( const int RoomX, const int RoomY, const int RoomZ )
{
	return RoomX + ( RoomY * m_MapSizeX ) + ( RoomZ * m_MapSizeX * m_MapSizeY );
}

inline void EldWorldGen::GetRoomCoords( const int RoomIndex, int& RoomX, int& RoomY, int& RoomZ )
{
...
bool EldWorldGen::FindFittingRooms( const ERoomExits ExitConfig, const SFeatureRoomDef& FeatureRoomDef, Array<SFittingRoom>& OutFittingRooms )
{
	PROFILE_FUNCTION;
...
bool EldWorldGen::Fits( const ERoomExits ExitConfig, const int Transform, const int RoomIndex, const bool AllowUnavailable )
{
...
bool EldWorldGen::IsAvailableNode( const int RoomIndex )
{
	return IsUnopenNode( RoomIndex ) || IsOpenNode( RoomIndex );
}

bool EldWorldGen::IsUnopenNode( const int RoomIndex )
{
	return m_Maze_UnopenRoomSet.Search( RoomIndex ).IsValid();
}

bool EldWorldGen::IsOpenNode( const int RoomIndex )
{
	return m_Maze_OpenRoomSet.Search( RoomIndex ).IsValid();
}

bool EldWorldGen::IsClosedNode( const int RoomIndex )
{
	return m_Maze_ClosedRoomSet.Search( RoomIndex ).IsValid();
}

bool EldWorldGen::IsLockedNode( const int RoomIndex )
{
	return m_Maze_LockedRoomSet.Search( RoomIndex ).IsValid();
}

void EldWorldGen::InsertAndLockRoom( const SRoomDef& RoomDef, const SFittingRoom& FittingRoom, const SFeatureRoomDef& FeatureRoomDef )
{
...
void EldWorldGen::Generate( const bool SimGeneration /*= false*/ )
{
	PROFILE_FUNCTION;
...
void EldWorldGen::AddFeatureRooms()
{
...
void EldWorldGen::InitializeMazeGen()
{
...
void EldWorldGen::GetRoomFilenameAndTransform( const SGenRoom& GenRoom, SimpleString& OutRoomFilename, ERoomXform& OutTransform, const HashedString& ElectedTheme )
{
...
void EldWorldGen::LookupRandomRoom( const SGenRoom& GenRoom, SimpleString& OutRoomFilename, ERoomXform& OutTransform )
{
...
void EldWorldGen::LookupRandomThemeRoom( const SGenRoom& GenRoom, SimpleString& OutRoomFilename, ERoomXform& OutTransform, const HashedString& ElectedTheme )
{
	ASSERT( m_RoomMap.Search( GenRoom.m_Exits ).IsValid() );
	const Array<SRoomDefLookup>& RoomsFittingExit = m_RoomMap[ GenRoom.m_Exits ];
...
void EldWorldGen::CopyMazeToWorld()
{
	XTRACE_FUNCTION;

	ASSERT( m_World );

	Array<c_uint8> RoomExits;
	RoomExits.Resize( m_NumRooms );
	for( uint RoomIndex = 0; RoomIndex < m_NumRooms; ++RoomIndex )
	{
		const SGenRoom& GenRoom = m_GeneratedRooms[ RoomIndex ];
		RoomExits[ RoomIndex ] = static_cast<c_uint8>( GenRoom.m_Exits );
	}

	m_World->SetMaze( RoomExits );
}

void EldWorldGen::PopulateWorld()
{
	PROFILE_FUNCTION;
	...
	for( uint RoomIndex = 0; RoomIndex < m_NumRooms; ++RoomIndex )
	{
		...
		if( m_SimmingGeneration )
		{
			...
			if( 0 == GenRoom.m_PrescribedFilename.Length() )
			{
				const ERoomExits UntransformedExits = static_cast<ERoomExits>( GetUntransformedExits( GenRoom.m_Exits, Transform ) );
				m_ExitStats[ UntransformedExits ]++;
			}
#endif
		}
		else
...
void EldWorldGen::ValidateVoxels() const
{
#if BUILD_DEV
	const vpos_t LoX = 1;
	const vpos_t LoY = 1;
...
void EldWorldGen::FilterPreparedSpawns()
{
...
void EldWorldGen::ResolveEntitySpawns()
{
	FOR_EACH_MAP( GroupIter, m_SpawnResolveGroups, HashedString, SSpawnResolveGroup )
	{
		SSpawnResolveGroup& ResolveGroup = GroupIter.GetValue();
...
void EldWorldGen::SpawnPreparedEntity( const SPreparedSpawn& PreparedSpawn )
{
...
#if BUILD_DEV
void EldWorldGen::ValidateSpawners()
{
...
void EldWorldGen::ValidateSpawnersInRoom( const EldRoom& Room, const SimpleString& RoomFilename )
{
	const Array<EldRoom::SSpawner>& Spawners = Room.GetSpawners();
	const uint NumSpawners = Spawners.Size();
	for( uint SpawnerIndex = 0; SpawnerIndex < NumSpawners; ++SpawnerIndex )
	{
		const EldRoom::SSpawner&	Spawner					= Spawners[ SpawnerIndex ];
...
void EldWorldGen::PrepareSpawnEntitiesFromRoom( const EldRoom& Room, const ERoomXform Transform, const uint RoomX, const uint RoomY, const uint RoomZ )
{
	PROFILE_FUNCTION;
	...
	for( uint SpawnerIndex = 0; SpawnerIndex < NumSpawners; ++SpawnerIndex )
	{
		...
		// (See also RemappedDef in eldtools.cpp)
		STATICHASH( Deprecated );
		const bool Deprecated = ConfigManager::GetBool( sDeprecated, false, sSpawnerDef );
		if( Deprecated )
		{
...
SimpleString EldWorldGen::TryGetHardSpawner( const SimpleString& BaseSpawnerDef )
{
	if( m_IsHardMode )
	{
...
SimpleString EldWorldGen::GetSubSpawnerDef( const SimpleString& SuperSpawnerDef )
{
...
float EldWorldGen::GetYaw( const int Direction )
{
...
float EldWorldGen::GetTransformedYaw( const int Direction, const int Transform )
{
	return GetYaw( GetTransformedExits( Direction, Transform ) );
}

Vector EldWorldGen::GetTransformedOffset( const Vector& Offset, const int Transform )
{
	switch( Transform )
	{
...
EldWorldGen::ERoomExits EldWorldGen::GetDirection( const c_uint8 Orientation ) const
{
...
void EldWorldGen::PrepareSpawnEntity( const SimpleString& SpawnerDef, const HashedString& ResolveGroupName, const vidx_t VoxelLocation, const ERoomExits Direction, const ERoomXform Transform, const uint RoomX, const uint RoomY, const uint RoomZ )
{
...
void EldWorldGen::GatherStats()
{
	...
	const SimpleString WorldDefName		= ReverseHash::ReversedHash( m_World->m_CurrentWorldDef );
	const SimpleString CurrentTimeDate	= TimeDate::GetTimeDateString();
	const SimpleString StatsFilename	= SimpleString::PrintF( "worldgen-stats-%s-%s.csv", WorldDefName.CStr(), CurrentTimeDate.CStr() );
	PrintStats( FileStream( StatsFilename.CStr(), FileStream::EFM_Write ) );

	ValidateSpawners();
#endif
}

void EldWorldGen::ResetStats()
{
#if BUILD_DEV
	m_RoomStats.Clear();
	m_ExitStats.Clear();
...
void EldWorldGen::PrintStats( const IDataStream& Stream )
{
	Unused( Stream );
	...
	struct SSortedRoomStats
	{
		...
		HashedString	m_RoomHash;
		uint			m_NumUses;

		static bool Sort( const SSortedRoomStats& A, const SSortedRoomStats& B )
		{
			return A.m_NumUses > B.m_NumUses;
		}
	};

	Array<SSortedRoomStats> SortedRoomStats;
	SortedRoomStats.Reserve( m_RoomStats.Size() + m_ExitStats.Size() );

	FOR_EACH_MAP( StatsIter, m_RoomStats, HashedString, uint )
	{
		SSortedRoomStats& NewRoomStats	= SortedRoomStats.PushBack();
		NewRoomStats.m_RoomHash			= StatsIter.GetKey();
		NewRoomStats.m_NumUses			= StatsIter.GetValue();
	}

	FOR_EACH_MAP( StatsIter, m_ExitStats, ERoomExits, uint )
	{
		SSortedRoomStats& NewRoomStats	= SortedRoomStats.PushBack();
		NewRoomStats.m_RoomHash			= HashedString( GetExitString( StatsIter.GetKey() ) );
		NewRoomStats.m_NumUses			= StatsIter.GetValue();
	}

	SortedRoomStats.QuickSort( SSortedRoomStats::Sort );

	const float fRcpNumStatsIterations = 1.0f / static_cast<float>( NumStatsIterations );

	Stream.PrintF( "Room Filename,Num Uses (%d iterations),Frequency\n", NumStatsIterations );
	FOR_EACH_ARRAY( SortedStatsIter, SortedRoomStats, SSortedRoomStats )
	{
		const SSortedRoomStats& RoomStats = SortedStatsIter.GetValue();
		const SimpleString ReversedFilename = ReverseHash::ReversedHash( RoomStats.m_RoomHash );
		const float Frequency = static_cast<float>( RoomStats.m_NumUses ) * fRcpNumStatsIterations;
		Stream.PrintF( "%s,%d,%f\n", ReversedFilename.CStr(), RoomStats.m_NumUses, Frequency );
	}
#endif
}

// Convert a room voxel index into a world voxel index, given the room position.
vidx_t EldWorldGen::GetWorldVoxelIndex( const vidx_t RoomVoxelIndex, const uint RoomX, const uint RoomY, const uint RoomZ )
{
...
void EldWorldGen::SetVoxelsFromRoom( const EldRoom& Room, const ERoomXform Transform, const uint RoomX, const uint RoomY, const uint RoomZ )
{
	PROFILE_FUNCTION;
	...
	for( vpos_t PosZ = BaseZ; PosZ < HighZ; ++PosZ )
	{
		for( vpos_t PosY = BaseY; PosY < HighY; ++PosY )
		{
			for( vpos_t PosX = BaseX; PosX < HighX; ++PosX, ++RoomVoxelIndex )
			{
				...
				// Calling inline functions in EldWorld. This could cause linker errors.
...
int EldWorldGen::GetTransformedRoomVoxelIndex( const int RoomVoxelIndex, const int Transform, const uint RoomSize )
{
...
int EldWorldGen::GetUntransformedRoomVoxelIndex( const int RoomVoxelIndex, const int Transform, const uint RoomSize )
{
	int X = RoomVoxelIndex % RoomSize;
	int Y = ( RoomVoxelIndex / RoomSize ) % RoomSize;
	int Z = ( ( RoomVoxelIndex / RoomSize ) / RoomSize ) % RoomSize;
...
void EldWorldGen::GetNeighborIndices( const int RoomIndex, SNeighborIndices& OutIndices )
{
...
void EldWorldGen::FindOpenLoopOpportunities( Array<SOpenLoopOpportunity>& OutOpportunities )
{
	const int HighX = static_cast<int>( m_MapSizeX - 1 );
	const int HighY = static_cast<int>( m_MapSizeY - 1 );
	const int HighZ = static_cast<int>( m_MapSizeZ - 1 );
...
void EldWorldGen::MazeOpenLoops()
{
...
void EldWorldGen::MazeExpansion()
{
	PROFILE_FUNCTION;
...
void EldWorldGen::TryExpandMaze( const ERoomExits Exits, const int OpenRoomIndex, const SNeighborIndices& Neighbors, const bool OpenRoom )
{
...
void EldWorldGen::ExpandMaze( const int OpenRoomIndex, const int UnopenRoomIndex, const ERoomExits Direction, const ERoomExits OppositeDirection, const bool OpenRoom )
{
