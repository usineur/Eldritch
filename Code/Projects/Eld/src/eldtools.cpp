...
void EldTools::InitializeFromDefinition( const SimpleString& DefinitionName )
{
...
void EldTools::Reinitialize( const uint RoomScalarX /*= 1*/, const uint RoomScalarY /*= 1*/, const uint RoomScalarZ /*= 1*/ )
{
	ShutDown();
...
void EldTools::InitializeHelpMeshes()
{
	...
	m_AltHelpMesh = pRenderer->Print( AltHelpText.CStr(), pRenderer->GetFontManager()->GetFont( DEFAULT_FONT_TAG ), SRect(), 0 );
}

void EldTools::CreateGridMesh( const Array<Vector>& Corners, const uint Color )
{
...
void EldTools::InitializeGridMeshes()
{
...
void EldTools::SetVoxel( const vidx_t VoxelIndex, const vval_t VoxelValue )
{
...
vval_t EldTools::GetPaletteValue()
{
	if( m_Palette.Size() )
	{
...
void EldTools::InitializeSpawnerMeshes()
{
	...
	for( uint CategoryIndex = 0; CategoryIndex < NumCategories; ++CategoryIndex )
	{
		...
		Mesh* const pCategoryMesh = pRenderer->Print( SpawnerCategory.CStr(), pRenderer->GetFontManager()->GetFont( DEFAULT_FONT_TAG ), Rect, 0 );
	...
	for( uint SpawnerDefIndex = 0; SpawnerDefIndex < NumSpawnerDefs; ++SpawnerDefIndex )
	{
		...
		Mesh* const pSpawnerDefMesh = pRenderer->Print( SpawnerDef.CStr(), pRenderer->GetFontManager()->GetFont( DEFAULT_FONT_TAG ), Rect, 0 );
...
void EldTools::InitializePaletteMeshes()
{
	const Map<vval_t, EldWorld::SVoxelDef>& VoxelDefs = m_World->GetWorldDef().m_VoxelDefs;

	FOR_EACH_MAP( VoxelDefIter, VoxelDefs, vval_t, EldWorld::SVoxelDef )
	{
		CreatePaletteMesh( VoxelDefIter.GetKey(), VoxelDefIter.GetValue() );
	}
}

void EldTools::CreatePaletteMesh( const vval_t VoxelValue, const EldWorld::SVoxelDef& VoxelDef )
{
	IRenderer* const pRenderer = m_Framework->GetRenderer();
...
#undef PUSH_INDICES
	...
	IVertexDeclaration* const	pVertexDeclaration	= pRenderer->GetVertexDeclaration( VD_POSITIONS | VD_UVS | VD_FLOATCOLORS );
	...
	InitStruct.FloatColors	= Colors.GetData();
	...
	pPaletteMesh->SetMaterialDefinition( "Rm_Material_WorldToolsHUD", pRenderer );
...
#if BUILD_DEBUG
	pPaletteMesh->m_Name = "Palette Voxel";
#endif
...
void EldTools::RefreshVoxelMeshes()
{
...
void EldTools::RefreshVoxelMesh( const vidx_t VoxelIndex )
{
	ASSERT( VoxelIndex < m_NumVoxels );
...
void EldTools::CreateVoxelMesh( const vidx_t VoxelIndex )
{
	ASSERT( VoxelIndex < m_NumVoxels );
	...
	const EldWorld::SVoxelDef& VoxelDef	= m_World->GetVoxelDef( VoxelValue );
...
#undef PUSH_INDICES
	...
	const SimpleString& WorldTileset = m_World->GetWorldDef().m_Tileset;
	pVoxelMesh->SetTexture( 0, pRenderer->GetTextureManager()->GetTextureNoMips( WorldTileset.CStr() ) );
...
void EldTools::DestroyVoxelMesh( const vidx_t VoxelIndex )
{
	ASSERT( VoxelIndex < m_VoxelMeshes.Size() );
	SafeDelete( m_VoxelMeshes[ VoxelIndex ] );
}

uint EldTools::GetVoxelIndexForCoords( vpos_t X, vpos_t Y, vpos_t Z )
{
...
uint EldTools::SafeGetVoxelIndexForCoords( const Vector& Location )
{
...
uint EldTools::SafeGetVoxelIndexForCoords( vpos_t X, vpos_t Y, vpos_t Z )
{
...
vval_t EldTools::SafeGetVoxel( const vidx_t VoxelIndex )
{
...
void EldTools::GetCoordsForVoxel( const vidx_t VoxelIndex, vpos_t& X, vpos_t& Y, vpos_t& Z )
{
	ASSERT( VoxelIndex < m_NumVoxels );
...
Vector EldTools::GetCoordsForVoxel( const vidx_t VoxelIndex )
{
	vpos_t X, Y, Z;
...
bool EldTools::IsInToolMode() const
{
	return m_ToolMode;
}

void EldTools::ToggleToolMode()
{
...
void EldTools::Tick( const float DeltaTime )
{
	Unused( DeltaTime );
...
void EldTools::TickInput( const float DeltaTime )
{
...
void EldTools::TickPaletteInput()
{
	Keyboard* const pKeyboard = m_Framework->GetKeyboard();
...
void EldTools::TickSpawnersInput()
{
	Keyboard* const pKeyboard = m_Framework->GetKeyboard();
	Mouse* const pMouse = m_Framework->GetMouse();
...
void EldTools::SetSubTool( const ESubTool SubTool )
{
	Mouse* const pMouse = m_Framework->GetMouse();
...
void EldTools::TickNormalInput( const float DeltaTime )
{
	...
	m_CameraRotationalVelocity.Yaw			-= kMouseAdjustment * pMouse->GetPosition( Mouse::EA_X );
	m_CameraRotationalVelocity.Pitch		-= kMouseAdjustment * pMouse->GetPosition( Mouse::EA_Y );
}
...
void EldTools::AdjustBrush( const uint Axis, const int BaseChange, const int SizeChange )
{
...
void EldTools::ExpandBrushTo( const vidx_t VoxelIndex )
{
	ASSERT( VoxelIndex < m_NumVoxels );
...
void EldTools::ShrinkBrushTo( const vidx_t VoxelIndex )
{
	ASSERT( VoxelIndex < m_NumVoxels );
...
void EldTools::FillBrush()
{
	FillBrush( true );
}

void EldTools::EraseBrush()
{
	FillBrush( false, 0 );
}

void EldTools::FillBrush( const bool UsePalette, const vval_t VoxelValue /*= 0*/ )
{
...
void EldTools::TraceVoxel( vidx_t& HitVoxel, vidx_t& FacingVoxel )
{
	...
	{
		const Vector&	HitPoint		= Info.m_Out_Intersection;
...
bool EldTools::TraceBoundPlanes( const Segment& TraceSegment, CollisionInfo& Info )
{
...
bool EldTools::Trace( const Segment& TraceSegment, CollisionInfo& Info )
{
	CollisionInfo MinInfo;
...
void EldTools::TryClear( const bool WithFloor, const bool WithShell )
{
...
void EldTools::Clear( const bool WithFloor, const bool WithShell )
{
	m_Spawners.Clear();
...
void EldTools::TickRender()
{
	...
	EldTargetManager* const pTargetManager = m_Framework->GetTargetManager();
	STATIC_HASHED_STRING( Primary );
	IRenderTarget* const pPrimaryRenderTarget = pTargetManager->GetRenderTarget( sPrimary );
...
void EldTools::Save( const IDataStream& Stream )
{
	EldRoom TempRoom;
	...
	FOR_EACH_MAP( SpawnerIter, m_Spawners, vidx_t, SPlacedSpawner )
	{
		EldRoom::SSpawner& Spawner = TempRoom.m_Spawners.PushBack();
...
void EldTools::Load( const IDataStream& Stream )
{
	EldRoom TempRoom;
	...
	{
		const EldRoom::SSpawner& Spawner = TempRoom.m_Spawners[ SpawnerIndex ];
...
float EldTools::GetYaw( const c_uint8 Orientation ) const
{
...
c_uint8 EldTools::GetOrientation( const HashedString& Orientation ) const
{
	STATIC_HASHED_STRING( North );
