...
/*virtual*/ void WBCompEldDoor::Tick( const float DeltaTime )
{
	XTRACE_FUNCTION;
...
void WBCompEldDoor::Lock()
{
	...
	if( !m_Open )
	{
		EldNav::GetInstance()->UpdateWorldFromEntity( pCollision, false );
	}
	...
	if( !m_Open )
	{
		EldNav::GetInstance()->UpdateWorldFromEntity( pCollision, true );
	}
