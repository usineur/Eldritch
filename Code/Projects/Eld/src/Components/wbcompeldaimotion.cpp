...
void WBCompEldAIMotion::InitializeFromDefinition( const SimpleString& DefinitionName )
{
	...
	STATICHASH( BypassPathing );
	m_BypassPathing = ConfigManager::GetInheritedBool( sBypassPathing, false, sDefinitionName );
...
/*virtual*/ void WBCompEldAIMotion::Tick( const float DeltaTime )
{
	XTRACE_FUNCTION;
...
void WBCompEldAIMotion::InternalStartMove( const Vector& Destination, const WBEntity* const pDestinationEntity )
{
	...
	EldNav* const			pNav				= EldNav::GetInstance();
	ASSERT( pNav );
	...
	WBCompEldCollision* const	pCollision			= GET_WBCOMP( pEntity, EldCollision );
	const bool					BypassPathing		= m_BypassPathing || ( m_IsFlying && pCollision == NULL );
	...
	else
	{
		...
		Params.m_MotionType			= m_IsFlying ? EldNav::EMT_Flying : EldNav::EMT_Walking;
		...
		const EldNav::EPathStatus Status = m_Wander ? pNav->Wander( Params, m_Path ) : pNav->FindPath( Params, m_Path );
		if( Status == EldNav::EPS_NoPathFound )
		{
			return;
		}
