...
/*virtual*/ void WBCompEldFrobber::Tick( const float DeltaTime )
{
	XTRACE_FUNCTION;
...
void WBCompEldFrobber::TryFrob( const int InputEdge )
{
	...
	DEBUGASSERT(
		pTargetFrobbable == NULL ||
		pTargetFrobbable == FindTargetFrobbable() );
...
WBEntity* WBCompEldFrobber::FindTargetFrobbable() const
{
	...
	for( uint FrobbableIndex = 0; FrobbableIndex < NumFrobbables; ++FrobbableIndex )
	{
		...
		{
			if( Info.m_Out_HitT <= CollidableT ) // Check if trace was blocked by some collision
			{
				if( Info.m_Out_HitT < NearestT || pNearestEntity == NULL )
				{
