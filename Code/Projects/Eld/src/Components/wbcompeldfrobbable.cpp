...
/*virtual*/ void WBCompEldFrobbable::HandleEvent( const WBEvent& Event )
{
	...
	if( EventName == sMarshalFrob )
	{
		...
		const uint Input = Event.GetInt( sInputEdge );
	...
	else if( EventName == sOnMeshUpdated )
	{
		...
		EldMesh* const pMesh = pMeshComponent->GetMesh();
		ASSERT( pMesh );
...
void WBCompEldFrobbable::MarshalFrob( WBEntity* const pFrobber, const uint Input )
{
