...
class WBCompEldPlayer : public WBEldComponent, public IInputSystemObserver
{
public:
	WBCompEldPlayer();
	virtual ~WBCompEldPlayer();

	DEFINE_WBCOMP( EldPlayer, WBEldComponent );
...
private:
	...
	float			m_PreviousPlayTime;			// Serialized; the amount of time spent in prior levels, added to GetTime() to get total play time
