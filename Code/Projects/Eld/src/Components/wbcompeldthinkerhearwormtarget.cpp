...
/*virtual*/ void WBCompEldThinkerHearWormTarget::Tick( const float DeltaTime )
{
	XTRACE_FUNCTION;
	...
	FOR_EACH_MAP( KnowledgeIter, KnowledgeMap, WBEntityRef, WBCompRodinKnowledge::TKnowledge )
	{
		...
		const EldFactions::EFactionCon Con = WBCompEldFaction::GetCon( pEntity, pKnowledgeEntity );
		if( Con == EldFactions::EFR_Friendly )
		{
			continue;
		}

		// Filter out knowledge entities that are neutral and not yet regarded as hostile
		if( Con == EldFactions::EFR_Neutral )
		{
