	...
	do
	{
		...
		if( GetWorld()->Sweep( SweepSegment, m_HalfExtents, Info ) )
		{
			...
			if( Info.m_Out_Plane.m_Normal.LengthSquared() > 0.0f )
			{
				...
				const bool LandedOnFloor = ( Info.m_Out_Plane.m_Normal.Dot( UpVector ) > 0.9f );
				...
				{
					OnCollided( Info.m_Out_Plane.m_Normal, pHitEntity );
				}
			}
			else
			{
...
void WBCompEldCollision::ConditionalSetNavBlocking( const bool NavBlocking )
{
	...
	{
		EldNav::GetInstance()->UpdateWorldFromEntity( this, NavBlocking );
