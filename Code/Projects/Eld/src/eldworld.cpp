...
// This simplifies a bunch of stuff and should never need to change for Eld.
...
// Map from direction bits to neighbor enums
static const vxnb_t kDirectionToNeighborMap[] =
{
	ENeighborNone,	// EDirectionNone
	ENeighborRight,	// EDirectionRight
	ENeighborLeft,	// EDirectionLeft
	0,				// invalid (R/L)
	ENeighborFront,	// EDirectionFront
	ENeighborRFC,	// EDirectionRight | EDirectionFront
	ENeighborLFC,	// EDirectionLeft | EDirectionFront
	0,				// invalid (R/L)
	ENeighborBack,	// EDirectionBack
	ENeighborRBC,	// EDirectionRight | EDirectionBack
	ENeighborLBC,	// EDirectionLeft | EDirectionBack
	0,				// invalid (R/L)
	0,				// invalid (F/B)
	0,				// invalid (F/B)
	0,				// invalid (F/B)
	0,				// invalid (F/B)
	ENeighborUp,	// EDirectionUp
	ENeighborRCU,	// EDirectionRight | EDirectionUp
	ENeighborLCU,	// EDirectionLeft | EDirectionUp
	0,				// invalid (R/L)
	ENeighborCFU,	// EDirectionFront | EDirectionUp
	ENeighborRFU,	// EDirectionRight | EDirectionFront | EDirectionUp
	ENeighborLFU,	// EDirectionLeft | EDirectionFront | EDirectionUp
	0,				// invalid (R/L)
	ENeighborCBU,	// EDirectionBack | EDirectionUp
	ENeighborRBU,	// EDirectionRight | EDirectionBack | EDirectionUp
	ENeighborLBU,	// EDirectionLeft | EDirectionBack | EDirectionUp
	0,				// invalid (R/L)
	0,				// invalid (F/B)
	0,				// invalid (F/B)
	0,				// invalid (F/B)
	0,				// invalid (F/B)
	ENeighborDown,	// EDirectionDown
	ENeighborRCD,	// EDirectionRight | EDirectionDown
	ENeighborLCD,	// EDirectionLeft | EDirectionDown
	0,				// invalid (R/L)
	ENeighborCFD,	// EDirectionFront | EDirectionDown
	ENeighborRFD,	// EDirectionRight | EDirectionFront | EDirectionDown
	ENeighborLFD,	// EDirectionLeft | EDirectionFront | EDirectionDown
	0,				// invalid (R/L)
	ENeighborCBD,	// EDirectionBack | EDirectionDown
	ENeighborRBD,	// EDirectionRight | EDirectionBack | EDirectionDown
	ENeighborLBD,	// EDirectionLeft | EDirectionBack | EDirectionDown
	0,				// invalid (R/L)
	0,				// invalid (F/B)
	0,				// invalid (F/B)
	0,				// invalid (F/B)
...
EldWorld::SVoxelLight::SVoxelLight()
:	m_Color()
,	m_Radius( 0.0f )
{
}
...
void EldWorld::InitializeVoxelPacking()
{
...
void EldWorld::PackVoxels()
{
	const uint UnpackedVoxelSize = m_NumVoxels * sizeof( vval_t );
	uLong PackedVoxelSize = compressBound( UnpackedVoxelSize );
	...
	m_PackedVoxels.Resize( static_cast<c_uint32>( PackedVoxelSize ) );
}

void EldWorld::UnpackVoxels()
{
	const uint UnpackedVoxelSize = m_NumVoxels * sizeof( vval_t );
	uLong DestinationSize = static_cast<uLong>( UnpackedVoxelSize );	// zlib writes the size of the compressed buffer (m_PackedVoxels.Size()?) to this for some reason.
	uncompress( m_VoxelMap.GetData(), &DestinationSize, m_PackedVoxels.GetData(), m_PackedVoxels.Size() );
}

void EldWorld::InitializeConfig( const SimpleString& DefinitionName )
{
...
void EldWorld::InitializeWorldDefConfig( const SimpleString& WorldDefinitionName )
{
	...
	NewWorldDef.m_WorldGen = new EldWorldGen;
	NewWorldDef.m_WorldGen->Initialize( WorldGenDefName );
}

void EldWorld::InitializeLightConfig( const SimpleString& LightDefinitionName, SWorldDef& WorldDef )
{
...
void EldWorld::InitializeFogConfig( const SimpleString& FogDefinitionName, SWorldDef& WorldDef )
{
...
void EldWorld::Initialize()
{
	m_Framework = EldFramework::GetInstance();
...
void EldWorld::CreateImpenetrableOuterWalls()
{
...
void EldWorld::GetMeshCoords( const uint MeshIndex, uint& OutX, uint& OutY, uint& OutZ ) const
{
...
uint EldWorld::GetMeshIndex( const uint MeshX, const uint MeshY, const uint MeshZ ) const
{
	return MeshX + ( MeshY * m_NumMeshesX ) + ( MeshZ * m_NumMeshesX * m_NumMeshesY );
}

void EldWorld::BuildMeshesForChangedVoxels( const Set<vidx_t>& ChangedVoxels )
{
	PROFILE_FUNCTION;
...
void EldWorld::BuildMesh( const uint MeshIndex )
{
	PROFILE_FUNCTION;
	...
	IVertexDeclaration*		VertexDeclaration	= m_Framework->GetRenderer()->GetVertexDeclaration( VD_POSITIONS | VD_FLOATCOLORS | VD_UVS );
	...
	InitStruct.FloatColors	= Buffers.Colors.GetData();
...
#endif
	{
		...
		NewWorldMesh->SetTexture( 0, m_Framework->GetRenderer()->GetTextureManager()->GetTextureNoMips( WorldTileset.CStr() ) );
	}
...
Vector EldWorld::ApplyDisplacementNoise( const Vector& Location, const SWorldDef& WorldDef ) const
{
...
inline Vector4 EldWorld::GetColorForNeighbor( const vidx_t VoxelIndex, const vdir_t NeighborDirection, const uint IrradianceDirection ) const
{
	const vidx_t			NeighborIndex	= GetIndexNeighbor( VoxelIndex, NeighborDirection );
	const SVoxelIrradiance&	Irradiance		= GetIrradiance( NeighborIndex );
	return Irradiance.m_Light[ IrradianceDirection ];
}

Vector4 EldWorld::GetColorForNeighbors( const Vector4& VoxelColor, const vidx_t VoxelIndex, const vxnb_t Neighbors, const vdir_t Directions[], const uint IrradianceDirection ) const
{
	static const float AvgMultiplier[] = { 1.0f, 1.0f, 0.5f, 0.33f, 0.25f };

	Vector4	Color		= VoxelColor;
	uint	NumElements	= 1;

	for( uint DirectionIndex = 0; DirectionIndex < 3; ++DirectionIndex )
	{
		const vdir_t	Direction	= Directions[ DirectionIndex ];
		const vxnb_t	Neighbor	= kDirectionToNeighborMap[ Direction ];
		if( !HasAnyNeighbors( Neighbors, Neighbor ) )
		{
			++NumElements;
			Color += GetColorForNeighbor( VoxelIndex, Direction, IrradianceDirection );
		}
	}

	return Color * AvgMultiplier[ NumElements ];
}

void EldWorld::BuildVertsForVoxel( const uint X, const uint Y, const uint Z, SVoxelMeshBuffers& Buffers ) const
{
	...
	{
		const EldWorld::SVoxelDef& VoxelDef = GetVoxelDef( Voxel );
...
#define ADD_MIDPOINT_FIXED_DISPLACEMENT( x )
		...
		if( !HasAnyNeighbors( Neighbors, ENeighborRight ) )
		{
			...
			static const vdir_t NeighborDirections[][3] =
			{
				{ EDirectionRBC, EDirectionRBD, EDirectionRCD },
				{ EDirectionRFC, EDirectionRFD, EDirectionRCD },
				{ EDirectionRBC, EDirectionRBU, EDirectionRCU },
				{ EDirectionRFC, EDirectionRFU, EDirectionRCU },
			};
			const Vector4 GlobalColor	= m_GlobalLight.m_Light[ IRRDIR_Right ];
			const Vector4 FaceColor		= GetColorForNeighbor( VoxelIndex, EDirectionRight, IRRDIR_Right );
			Colors[0] = GlobalColor + GetColorForNeighbors( FaceColor, VoxelIndex, Neighbors, NeighborDirections[0], IRRDIR_Right );
			Colors[1] = GlobalColor + GetColorForNeighbors( FaceColor, VoxelIndex, Neighbors, NeighborDirections[1], IRRDIR_Right );
			Colors[2] = GlobalColor + GetColorForNeighbors( FaceColor, VoxelIndex, Neighbors, NeighborDirections[2], IRRDIR_Right );
			Colors[3] = GlobalColor + GetColorForNeighbors( FaceColor, VoxelIndex, Neighbors, NeighborDirections[3], IRRDIR_Right );

			PushColors( Colors, m_AOColor, AO, Buffers );
		...
		if( !HasAnyNeighbors( Neighbors, ENeighborLeft ) )
		{
			...
			static const vdir_t NeighborDirections[][3] =
			{
				{ EDirectionLBC, EDirectionLBD, EDirectionLCD },
				{ EDirectionLFC, EDirectionLFD, EDirectionLCD },
				{ EDirectionLBC, EDirectionLBU, EDirectionLCU },
				{ EDirectionLFC, EDirectionLFU, EDirectionLCU },
			};
			const Vector4 GlobalColor	= m_GlobalLight.m_Light[ IRRDIR_Left ];
			const Vector4 FaceColor		= GetColorForNeighbor( VoxelIndex, EDirectionLeft, IRRDIR_Left );
			Colors[0] = GlobalColor + GetColorForNeighbors( FaceColor, VoxelIndex, Neighbors, NeighborDirections[0], IRRDIR_Left );
			Colors[1] = GlobalColor + GetColorForNeighbors( FaceColor, VoxelIndex, Neighbors, NeighborDirections[1], IRRDIR_Left );
			Colors[2] = GlobalColor + GetColorForNeighbors( FaceColor, VoxelIndex, Neighbors, NeighborDirections[2], IRRDIR_Left );
			Colors[3] = GlobalColor + GetColorForNeighbors( FaceColor, VoxelIndex, Neighbors, NeighborDirections[3], IRRDIR_Left );
		...
		if( !HasAnyNeighbors( Neighbors, ENeighborFront ) )
		{
			...
			static const vdir_t NeighborDirections[][3] =
			{
				{ EDirectionLFC, EDirectionLFD, EDirectionCFD },
				{ EDirectionRFC, EDirectionRFD, EDirectionCFD },
				{ EDirectionLFC, EDirectionLFU, EDirectionCFU },
				{ EDirectionRFC, EDirectionRFU, EDirectionCFU },
			};
			const Vector4 GlobalColor	= m_GlobalLight.m_Light[ IRRDIR_Front ];
			const Vector4 FaceColor		= GetColorForNeighbor( VoxelIndex, EDirectionFront, IRRDIR_Front );
			Colors[0] = GlobalColor + GetColorForNeighbors( FaceColor, VoxelIndex, Neighbors, NeighborDirections[0], IRRDIR_Front );
			Colors[1] = GlobalColor + GetColorForNeighbors( FaceColor, VoxelIndex, Neighbors, NeighborDirections[1], IRRDIR_Front );
			Colors[2] = GlobalColor + GetColorForNeighbors( FaceColor, VoxelIndex, Neighbors, NeighborDirections[2], IRRDIR_Front );
			Colors[3] = GlobalColor + GetColorForNeighbors( FaceColor, VoxelIndex, Neighbors, NeighborDirections[3], IRRDIR_Front );
		...
		if( !HasAnyNeighbors( Neighbors, ENeighborBack ) )
		{
			...
			static const vdir_t NeighborDirections[][3] =
			{
				{ EDirectionLBC, EDirectionLBD, EDirectionCBD },
				{ EDirectionRBC, EDirectionRBD, EDirectionCBD },
				{ EDirectionLBC, EDirectionLBU, EDirectionCBU },
				{ EDirectionRBC, EDirectionRBU, EDirectionCBU },
			};
			const Vector4 GlobalColor	= m_GlobalLight.m_Light[ IRRDIR_Back ];
			const Vector4 FaceColor		= GetColorForNeighbor( VoxelIndex, EDirectionBack, IRRDIR_Back );
			Colors[0] = GlobalColor + GetColorForNeighbors( FaceColor, VoxelIndex, Neighbors, NeighborDirections[0], IRRDIR_Back );
			Colors[1] = GlobalColor + GetColorForNeighbors( FaceColor, VoxelIndex, Neighbors, NeighborDirections[1], IRRDIR_Back );
			Colors[2] = GlobalColor + GetColorForNeighbors( FaceColor, VoxelIndex, Neighbors, NeighborDirections[2], IRRDIR_Back );
			Colors[3] = GlobalColor + GetColorForNeighbors( FaceColor, VoxelIndex, Neighbors, NeighborDirections[3], IRRDIR_Back );
		...
		if( !HasAnyNeighbors( Neighbors, ENeighborUp ) )
		{
			...
			static const vdir_t NeighborDirections[][3] =
			{
				{ EDirectionLCU, EDirectionLBU, EDirectionCBU },
				{ EDirectionRCU, EDirectionRBU, EDirectionCBU },
				{ EDirectionLCU, EDirectionLFU, EDirectionCFU },
				{ EDirectionRCU, EDirectionRFU, EDirectionCFU },
			};
			const Vector4 GlobalColor	= m_GlobalLight.m_Light[ IRRDIR_Up ];
			const Vector4 FaceColor		= GetColorForNeighbor( VoxelIndex, EDirectionUp, IRRDIR_Up );
			Colors[0] = GlobalColor + GetColorForNeighbors( FaceColor, VoxelIndex, Neighbors, NeighborDirections[0], IRRDIR_Up );
			Colors[1] = GlobalColor + GetColorForNeighbors( FaceColor, VoxelIndex, Neighbors, NeighborDirections[1], IRRDIR_Up );
			Colors[2] = GlobalColor + GetColorForNeighbors( FaceColor, VoxelIndex, Neighbors, NeighborDirections[2], IRRDIR_Up );
			Colors[3] = GlobalColor + GetColorForNeighbors( FaceColor, VoxelIndex, Neighbors, NeighborDirections[3], IRRDIR_Up );
		...
		if( !HasAnyNeighbors( Neighbors, ENeighborDown ) )
		{
			...
			static const vdir_t NeighborDirections[][3] =
			{
				{ EDirectionLCD, EDirectionLBD, EDirectionCBD },
				{ EDirectionRCD, EDirectionRBD, EDirectionCBD },
				{ EDirectionLCD, EDirectionLFD, EDirectionCFD },
				{ EDirectionRCD, EDirectionRFD, EDirectionCFD },
			};
			const Vector4 GlobalColor	= m_GlobalLight.m_Light[ IRRDIR_Down ];
			const Vector4 FaceColor		= GetColorForNeighbor( VoxelIndex, EDirectionDown, IRRDIR_Down );
			Colors[0] = GlobalColor + GetColorForNeighbors( FaceColor, VoxelIndex, Neighbors, NeighborDirections[0], IRRDIR_Down );
			Colors[1] = GlobalColor + GetColorForNeighbors( FaceColor, VoxelIndex, Neighbors, NeighborDirections[1], IRRDIR_Down );
			Colors[2] = GlobalColor + GetColorForNeighbors( FaceColor, VoxelIndex, Neighbors, NeighborDirections[2], IRRDIR_Down );
			Colors[3] = GlobalColor + GetColorForNeighbors( FaceColor, VoxelIndex, Neighbors, NeighborDirections[3], IRRDIR_Down );
...
inline void EldWorld::PushColors( const class Vector4* pColors, const Vector4& AOColor, const float AOValues[], SVoxelMeshBuffers& Buffers ) const
{
	const Vector4 Color0	= AOColor.LERP( AOValues[0], pColors[0] );
	const Vector4 Color1	= AOColor.LERP( AOValues[1], pColors[1] );
	const Vector4 Color2	= AOColor.LERP( AOValues[2], pColors[2] );
	const Vector4 Color3	= AOColor.LERP( AOValues[3], pColors[3] );
	const Vector4 Average	= 0.25f * ( Color0 + Color1 + Color2 + Color3 );
...
void EldWorld::PushUVs( const Vector2& BaseUV, const bool FlipU, const bool FlipV, struct SVoxelMeshBuffers& Buffers ) const
{
...
inline void EldWorld::PushIndices( index_t& BaseIndex, const bool CCW, struct SVoxelMeshBuffers& Buffers ) const
{
#if BUILD_DEBUG
	static const uint MaxIndex = 1 << ( 8 * sizeof( index_t ) );
	ASSERT( static_cast<uint>( BaseIndex + 4 ) < MaxIndex );
#endif
...
void EldWorld::Tick( const float DeltaTime )
{
	XTRACE_FUNCTION;
...
void EldWorld::Render() const
{
	XTRACE_FUNCTION;

	IRenderer* const pRenderer = m_Framework->GetRenderer();

	FOR_EACH_ARRAY( WorldMeshIter, m_WorldMeshes, Mesh* )
	{
		Mesh* const pWorldMesh = WorldMeshIter.GetValue();
		if( pWorldMesh )
		{
...
#if BUILD_DEV
void EldWorld::DebugRender() const
{
	IRenderer* const pRenderer = m_Framework->GetRenderer();
...
bool EldWorld::Sweep( const Segment& SweepSegment, const Vector& HalfExtents, CollisionInfo& Info ) const
{
	CollisionInfo MinInfo;

	ASSERT( Info.m_In_CollideWorld || Info.m_In_CollideEntities );
...
bool EldWorld::SweepVoxels( const Segment& SweepSegment, const Vector& HalfExtents, CollisionInfo& Info ) const
{
	CollisionInfo MinInfo;
...
bool EldWorld::SweepEntities( const Segment& SweepSegment, const Vector& HalfExtents, CollisionInfo& Info ) const
{
	// This gets called enough that the profiler hook is a cost. >_<
	//PROFILE_FUNCTION;

	const uint	CollideAsType	= Info.m_In_UserFlags & EECF_Mask_CollideAs;
	const uint	EntityType		= Info.m_In_UserFlags & EECF_Mask_EntityTypes;
...
bool EldWorld::Trace( const Ray& TraceRay, CollisionInfo& Info ) const
{
...
bool EldWorld::Trace( const Segment& TraceSegment, CollisionInfo& Info ) const
{
	// Since a sweep is just a trace against expanded boxes, a trace is just a sweep with zero extents!
	return Sweep( TraceSegment, Vector(), Info );
}

bool EldWorld::CheckClearance( const Vector& Location, const Vector& HalfExtents, CollisionInfo& Info ) const
{
...
bool EldWorld::PointCheck( const Vector& Location, CollisionInfo& Info ) const
{
...
bool EldWorld::LineCheck( const Vector& Start, const Vector& End, CollisionInfo& Info ) const
{
	const Segment TraceSegment( Start, End );
	return Sweep( TraceSegment, Vector(), Info );
}

bool EldWorld::SweepCheck( const Vector& Start, const Vector& End, const Vector& HalfExtents, CollisionInfo& Info ) const
{
	const Segment TraceSegment( Start, End );
	return Sweep( TraceSegment, HalfExtents, Info );
}

bool EldWorld::FindSpot( Vector& InOutSpot, const Vector& Extents, CollisionInfo& Info ) const
{
	PROFILE_FUNCTION;
...
inline bool EldWorld::HasAnyNeighbors( const vnbr_t VoxelNeighbors, const vnbr_t NeighborFlags ) const
{
	return ( VoxelNeighbors & NeighborFlags ) > 0;
}

inline bool EldWorld::HasAnyNeighbors( const vxnb_t VoxelExtendedNeighbors, const vxnb_t ExtendedNeighborFlags ) const
{
	return ( VoxelExtendedNeighbors & ExtendedNeighborFlags ) > 0;
}

inline uint EldWorld::CountNeighbors( const vxnb_t VoxelExtendedNeighbors, const vxnb_t ExtendedNeighborFlags ) const
{
...
inline float EldWorld::GetAOForNeighbors( const uint NumNeighbors ) const
{
...
/*inline*/ bool EldWorld::IsValidIndex( const vidx_t VoxelIndex ) const
{
	return VoxelIndex < m_NumVoxels;
}

inline bool EldWorld::IsValidX( const vpos_t X ) const
{
	return X >= 0 && X < m_NumVoxelsX;
}

inline bool EldWorld::IsValidY( const vpos_t Y ) const
{
	return Y >= 0 && Y < m_NumVoxelsY;
}

inline bool EldWorld::IsValidZ( const vpos_t Z ) const
{
	return Z >= 0 && Z < m_NumVoxelsZ;
}

// NOTE: This used to be inline, but not every compiler would link it properly.
/*inline*/ void EldWorld::GetCoords( const vidx_t VoxelIndex, vpos_t& OutX, vpos_t& OutY, vpos_t& OutZ ) const
{
	DEVASSERT( IsValidIndex( VoxelIndex ) );
...
// NOTE: This used to be inline, but not every compiler would link it properly.
/*inline*/ vidx_t EldWorld::GetIndex( const vpos_t X, const vpos_t Y, const vpos_t Z ) const
{
...
// NOTE: This used to be inline, but not every compiler would link it properly.
/*inline*/ vidx_t EldWorld::GetIndex( const Vector& Location ) const
{
...
Vector EldWorld::GetVoxelCenter( const Vector& Location ) const
{
	return Vector( Floor( Location.x ) + 0.5f, Floor( Location.y ) + 0.5f, Floor( Location.z ) + 0.5f );
}

Vector EldWorld::GetVoxelBase( const Vector& Location ) const
{
	return Vector( Floor( Location.x ), Floor( Location.y ), Floor( Location.z ) );
}

// NOTE: This used to be inline, but not every compiler would link it properly.
/*inline*/ Vector EldWorld::GetVoxelCenter( const vidx_t VoxelIndex ) const
{
	return GetVoxelBase( VoxelIndex ) + kVoxelHalfExtents;
}

inline Vector EldWorld::GetVoxelBase( const vidx_t VoxelIndex ) const
{
	vpos_t X, Y, Z;
...
inline vidx_t EldWorld::GetIndexNeighborRight( const vidx_t VoxelIndex ) const
{
	return VoxelIndex + 1;
}

inline vidx_t EldWorld::GetIndexNeighborLeft( const vidx_t VoxelIndex ) const
{
	return VoxelIndex - 1;
}

inline vidx_t EldWorld::GetIndexNeighborFront( const vidx_t VoxelIndex ) const
{
	return VoxelIndex + m_NumVoxelsX;
}

inline vidx_t EldWorld::GetIndexNeighborBack( const vidx_t VoxelIndex ) const
{
	return VoxelIndex - m_NumVoxelsX;
}

inline vidx_t EldWorld::GetIndexNeighborUp( const vidx_t VoxelIndex ) const
{
	return VoxelIndex + m_NumVoxelsX * m_NumVoxelsY;
}

inline vidx_t EldWorld::GetIndexNeighborDown( const vidx_t VoxelIndex ) const
{
	return VoxelIndex - m_NumVoxelsX * m_NumVoxelsY;
}

inline vidx_t EldWorld::GetIndexNeighbor( const vidx_t VoxelIndex, const vdir_t Direction ) const
{
...
// NOTE: This used to be inline, but not every compiler would link it properly.
/*inline*/ vval_t EldWorld::GetVoxel( const vidx_t VoxelIndex ) const
{
...
// NOTE: This used to be inline, but not every compiler would link it properly.
/*inline*/ void EldWorld::SetVoxel( const vidx_t VoxelIndex, const vval_t VoxelValue )
{
	DEBUGASSERT( IsValidIndex( VoxelIndex ) );
	m_VoxelMap[ VoxelIndex ] = VoxelValue;
}

inline vval_t EldWorld::SafeGetVoxel( const vidx_t VoxelIndex ) const
{
	return IsValidIndex( VoxelIndex ) ? GetVoxel( VoxelIndex ) : kInvalidVoxelValue;
}

inline void EldWorld::SafeSetVoxel( const vidx_t VoxelIndex, const vval_t VoxelValue )
{
...
inline const SVoxelIrradiance& EldWorld::GetIrradiance( const vidx_t VoxelIndex ) const
{
	DEBUGASSERT( IsValidIndex( VoxelIndex ) );
	return m_IrradianceMap[ VoxelIndex ];
}

inline SVoxelIrradiance& EldWorld::GetIrradiance( const vidx_t VoxelIndex )
{
	DEBUGASSERT( IsValidIndex( VoxelIndex ) );
	return m_IrradianceMap[ VoxelIndex ];
}

inline vnbr_t EldWorld::GetNeighbors( const vidx_t VoxelIndex ) const
{
	vnbr_t RetVal = 0;
...
inline vxnb_t EldWorld::GetExtendedNeighbors( const vidx_t VoxelIndex ) const
{
	vxnb_t RetVal = 0;
...
void EldWorld::SetMaze( const Array<c_uint8>& Maze )
{
	m_Maze = Maze;
}
...
void EldWorld::Save( const IDataStream& Stream )
{
	XTRACE_FUNCTION;
...
void EldWorld::Load( const IDataStream& Stream )
{
	XTRACE_FUNCTION;
...
void EldWorld::ClearLightInfluenceMap()
{
	PROFILE_FUNCTION;
...
AABB EldWorld::GetVoxelBox( const vidx_t VoxelIndex ) const
{
...
void EldWorld::ExpandBoxAroundVoxels( const Set<vidx_t>& Voxels, AABB& InOutBox ) const
{
	PROFILE_FUNCTION;
...
void EldWorld::SendOnWorldChangedEvent( const AABB& ChangedBox ) const
{
...
bool EldWorld::AddVoxelAt( const Vector& Location, const vval_t VoxelValue )
{
	CollisionInfo Info;
...
void EldWorld::RemoveVoxelAt( const Vector& Location )
{
	vidx_t VoxelIndex = GetIndex( Location );
	if( VoxelIndex == kInvalidVoxelIndex )
	{
...
void EldWorld::RemoveVoxelsAt( const Vector& Location, const float Radius )
{
	vidx_t VoxelIndex = GetIndex( Location );
	if( VoxelIndex == kInvalidVoxelIndex )
	{
...
void EldWorld::OnBoxChanged( const AABB& Box )
{
	PROFILE_FUNCTION;
...
void EldWorld::GatherLightInfluenceVoxels( Set<vidx_t>& Voxels, const vidx_t VoxelIndex ) const
{
	PROFILE_FUNCTION;
...
void EldWorld::GatherVoxelsInRadius( Set<vidx_t>& Voxels, const vidx_t VoxelIndex, const float Radius ) const
{
	PROFILE_FUNCTION;
...
void EldWorld::GatherVoxelsInBox( Set<vidx_t>& Voxels, const AABB& Box ) const
{
	PROFILE_FUNCTION;
...
void EldWorld::AddLightInfluence( const Set<vidx_t>& AffectedVoxels, const vidx_t LightVoxel )
{
	FOR_EACH_SET( VoxelIter, AffectedVoxels, vidx_t )
	{
		const vidx_t		VoxelIndex				= VoxelIter.GetValue();
...
void EldWorld::RemoveLightInfluence( const Set<vidx_t>& AffectedVoxels, const vidx_t LightVoxel )
{
	FOR_EACH_SET( VoxelIter, AffectedVoxels, vidx_t )
	{
		const vidx_t		VoxelIndex				= VoxelIter.GetValue();
...
bool EldWorld::AddLightAt( const Vector& Location, const float Radius, const Vector4& Color )
{
	vidx_t VoxelIndex = GetIndex( Location );
	if( VoxelIndex == kInvalidVoxelIndex )
	{
...
void EldWorld::RemoveLightAt( const Vector& Location )
{
	vidx_t VoxelIndex = GetIndex( Location );
	if( VoxelIndex == kInvalidVoxelIndex )
	{
...
void EldWorld::ComputeAllIrradiance()
{
	PROFILE_FUNCTION;
...
void EldWorld::ComputeIrradianceForVoxels( const Set<vidx_t>& Voxels )
{
	PROFILE_FUNCTION;
...
void EldWorld::ComputePartialIrradiance( vidx_t VoxelIndex, vidx_t LightVoxel, const SVoxelLight& Light )
{
	// This gets called enough that the profiler hook is a cost. >_<
	//PROFILE_FUNCTION;
...
inline float EldWorld::GetIrradianceIncidence( const Vector& LightDir, const Vector& IrradianceDir ) const
{
...
const SVoxelIrradiance& EldWorld::GetIrradianceAt( const Vector& Location ) const
{
	vidx_t VoxelIndex = GetIndex( Location );
	if( VoxelIndex == kInvalidVoxelIndex )
	{
...
SVoxelIrradiance EldWorld::BlendIrradiances( const Vector& LocationA, const Vector& LocationB ) const
{
...
void EldWorld::GetRoomDimensions( uint& SizeX, uint& SizeY, uint& SizeZ ) const
{
	SizeX = m_RoomSizeX;
	SizeY = m_RoomSizeY;
	SizeZ = m_RoomSizeZ;
...
const EldWorld::SWorldDef& EldWorld::GetWorldDef() const
{
...
const EldWorld::SVoxelDef& EldWorld::GetVoxelDef( const vval_t Voxel ) const
{
	const SWorldDef& CurrentWorldDef = GetWorldDef();
...
bool EldWorld::IsCollidable( const vval_t Voxel ) const
{
	return Voxel > kEmptyVoxelValue;
}

bool EldWorld::IsVisible( const vval_t Voxel ) const
{
	return Voxel > kEmptyVoxelValue;
}

void EldWorld::GatherStats()
{
	const SWorldDef& WorldDef = GetWorldDef();
...
ITexture* EldWorld::GetTileTexture() const
{
	IRenderer* const		pRenderer		= m_Framework->GetRenderer();
	TextureManager* const	pTextureManager	= pRenderer->GetTextureManager();
	const SimpleString&		WorldTileset	= GetWorldDef().m_Tileset;

	return pTextureManager->GetTextureNoMips( WorldTileset.CStr() );
}

void EldWorld::GetTileUVs( const uint TileIndex, Vector2& OutUVMin, Vector2& OutUVMax ) const
{
...
SimpleString EldWorld::GetSpawnerOverride( const SimpleString& OldSpawner ) const
{
	const SWorldDef& WorldDef = GetWorldDef();
...
int EldWorld::GetRoomIndex( const Vector& Location ) const
{
	// Note we simply drop the fractional part instead of rounding down.
...
void EldWorld::GetRoomCoords( const int RoomIndex, int& OutX, int& OutY, int& OutZ ) const
{
...
Vector EldWorld::GetWorldDimensions() const
{
