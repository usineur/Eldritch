...
/*virtual*/ bool Framework3D::TickSim( const float DeltaTime )
{
	...
	if( HasFocus() )
	{
		...
		if( m_Keyboard->OnRise( Keyboard::EB_F11 ) )
		{
			...
			if( m_Keyboard->IsHigh( Keyboard::EB_LeftShift ) || m_Keyboard->IsHigh( Keyboard::EB_LeftAlt ) )
			{
...
bool Framework3D::HasRequestedExit()
{
	...
	if( m_Keyboard->IsHigh( Keyboard::EB_LeftAlt ) && m_Keyboard->OnRise( Keyboard::EB_F4 ) )
	{
		return true;
	}
#endif
