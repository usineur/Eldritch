...
bool SimpleString::EndsWith( const SimpleString& String ) const
{
	const uint StringLength		= Length();
	const uint SubstringLength	= String.Length();
	...
	const uint StringOffset		= StringLength - SubstringLength;
	return ( strstr( m_String + StringOffset, String.m_String ) == m_String + StringOffset );
}
...
SimpleString SimpleString::Replace( const char* const Find, const char* const Replace ) const
{
	...
	const bool		Growing			= ( ReplaceLength > FindLength );
	const size_t	Difference		= Growing ? ( ReplaceLength - FindLength ) : ( FindLength - ReplaceLength );;
	...
	for( size_t Iterator = 0; Iterator < NewStringBuffer.Size(); )
	{
		...
		if( SubStr )
		{
			...
			else
			{
				for( size_t DifferenceIndex = 0; DifferenceIndex < Difference; ++DifferenceIndex )
				{
					NewStringBuffer.Remove( static_cast<uint>( Offset ) );
				}
			}
...
#define PERCENT_ENCODE_CHAR														\
	{																			\
		...
		const c_uint8		UnsignedChar	= static_cast<c_uint8>( c );		\
...
{
	SimpleString UpperString = m_String;

	const char CaseOffset = 'a' - 'A';
	for( char* pc = UpperString.m_String; *pc; ++pc )
	{
		char& c = *pc;
		if( c >= 'a' && c <= 'z' )
		{
...
/*static*/ SimpleString SimpleString::UnicodeToUTF8( const unicode_t Codepoint )
{
	Array<unicode_t> Unicode;
	Unicode.PushBack( Codepoint );
...
/*static*/ SimpleString SimpleString::UnicodeToUTF8( const Array<unicode_t>& Unicode )
{
