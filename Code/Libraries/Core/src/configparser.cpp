...
#define CONDITIONAL_OVERWRITE_ASSERT if( WarnIfOverwriting ) \
	if( ConfigManager::Exists( Name, Context ) ) \
	{ \
		PRINTF( "Config var %s:%s is overwriting an existing var!\n", Context.CStr(), Name.CStr() ); \
		WARNDESC( "Config var is overwriting an existing var, see log for details" ); \
	}
...
const char* SToken::m_TokenNames[] =
{
	"ET_None",
	"ET_Name",
	"ET_Equals",
...
void InnerParse( const char c, char& StrMark, Array<char>& TokenString, const int LineCount, SToken::ETokenType& InOutTokenType, bool* OutClosedString = NULL )
{
	...
	case SToken::ET_Int:
		if( c == '.' )
		{
			ASSERT( !TokenString.Find( 'x' ) );
			InOutTokenType = SToken::ET_Float;
		}
		else if( c == 'x' )
		{
			// We're parsing a hex string as int now
			ASSERT( '0' == TokenString.Last() );
		}
		else
		{
			ASSERT( IsHex( c ) );
	...
	case SToken::ET_String:
		...
		break;
	default:
		PRINTF( "Unexpected token of type %d (%s) at line %d\n", InOutTokenType, SToken::m_TokenNames[ InOutTokenType ], LineCount );
...
void ConfigParser::Parse( const IDataStream& Stream, const bool WarnIfOverwriting /*= false*/ )
{
	Array< SToken >	Tokens;
	SToken			Token;
	...
	for(;;)
	{
		...
		else if( c == '\\' && Token.m_TokenType == SToken::ET_String )
		{
			...
			else if( next == 'u' )
			{
				...
				const SimpleString UTF8String = SimpleString::UnicodeToUTF8( CodePointArray );
		...
		else if( c == '@' )
		{
			...
			else
			{
				...
				if( c == '@' )
				{
					...
					if( MacroToken.m_TokenString.Empty() )
					{
						PRINTF( "Empty macro inserted at line %d\n", LineCount );
						WARNDESC( "Empty macro inserted" );
					}
	...
	Array<SimpleString> ContextStack;
	ContextStack.SetDeflate( false );
	ContextStack.PushBack( "" );

	// Tokens are made, now create config vars
	for( uint i = 0; i < Tokens.Size(); ++i )
	{
		const SimpleString& Context = ContextStack.Last();
			...
			case SToken::ET_Bool:
				{
					...
					CONDITIONAL_OVERWRITE_ASSERT;
					ConfigManager::SetBool( Name, Value, Context );
				}
				break;
			case SToken::ET_Int:
				{
					int Value = static_cast<int>( strtol( ValueString, NULL, 0 ) );
		...
		{
			// HACKHACK: contexts beginning with + or - push and pop the context stack
			if( NameToken.m_TokenString.Size() > 0 &&
				'+' == NameToken.m_TokenString[0] )
			{
				ContextStack.PushBack( NameToken.m_TokenString.GetData() + 1 ); // Drop the + from the name
			}
			else if( NameToken.m_TokenString.Size() > 0 &&
				'-' == NameToken.m_TokenString[0] &&
				ContextStack.Size() > 1 ) // Never pop the bottom element
			{
				ContextStack.PopBack();
			}
			else
			{
				ContextStack.Clear();
				ContextStack.PushBack( NameToken.m_TokenString.GetData() );
			}
		}
		else
		{
...
void ConfigParser::ParseTiny( const IDataStream& Stream, const bool WarnIfOverwriting /*= false*/ )
{
	...
	while( !Stream.EOS() )
	{
		...
		else if( ParseState == EPS_ValuePart )
		{
			if( ( ( Type != SToken::ET_String || StringClosed ) && ( c == ' ' || c == '\t' ) ) || c == '\n' || c == '\0' )
			{
				...
				case SToken::ET_Int:
					{
						int Value = static_cast<int>( strtol( ValuePart.GetData(), NULL, 0 ) );
...
bool ConfigParser::EvaluateConditional( const IDataStream& Stream )
{
	...
	// NOTE: This does not support hex tokens, mostly because AFAIK I'm not using
	// this functionality anywhere anyway.
	// Evaluate
		...
		( LeftSideType == SToken::ET_Float && RightSideType == SToken::ET_Int ) )
	{
		...
		else if( LeftSideType == SToken::ET_Int )
		{
			...
			int RightSideInt = ( RightSideType == SToken::ET_Int ) ? RightSide.AsInt() : static_cast<int>( RightSide.AsFloat() );
		...
		else if( LeftSideType == SToken::ET_Float )
		{
			...
			float RightSideFloat = ( RightSideType == SToken::ET_Float ) ? RightSide.AsFloat() : static_cast<float>( RightSide.AsInt() );
