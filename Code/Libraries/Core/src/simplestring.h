...
typedef c_uint32	unicode_t;	// Represents a single Unicode code point in 4 bytes
...
public:
	...
	bool			EndsWith( const SimpleString& String ) const;
	...
	static SimpleString	UnicodeToUTF8( const Array<unicode_t>& Unicode );					// Converts array of Unicode code points to UTF-8 form
	static SimpleString	UnicodeToUTF8( const unicode_t CodePoint );							// Wrapper for UnicodeToUTF8 with a single codepoint
