...
int Window::Init( const char* Title, const char* ClassName, DWORD Style, DWORD ExStyle, int Width, int Height, HINSTANCE hInstance, WNDPROC WndProc /*= NULL*/, uint Icon /*= 0*/, int ScreenWidth /*= 0*/, int ScreenHeight /*= 0*/ )
{
	...
	m_WndClassEx.hCursor = (HCURSOR)LoadImage( NULL, IDC_ARROW, IMAGE_CURSOR, 0, 0, LR_SHARED );
...
#if BUILD_LINUX
		if( ( WindowFlags & ( SDL_WINDOW_FULLSCREEN | SDL_WINDOW_FULLSCREEN_DESKTOP ) ) != 0 )
		{
			// HACKHACK: Linux only! We don't have focus but we're fullscreen so pretend we do.
			return true;
		}
#endif
