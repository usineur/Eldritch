...
#define FOR_EACH_INDEX_FROM( idx, start, num )	for( uint idx = start; idx < num; ++idx )
...
public:
	...
	typedef bool ( *ArrayCompareFunc )( const C&, const C& );
	bool	FindCompare( const C& Item, ArrayCompareFunc CompareFunc, uint* const pOutIndex = NULL ) const
	{
		for( uint i = 0; i < m_Size; ++i )
		{
			if( CompareFunc( m_Array[i], Item ) )
			{
				if( pOutIndex )
				{
					*pOutIndex = i;
	...
	bool	FindCompare( const C& Item, ArrayCompareFunc CompareFunc, uint& OutIndex ) const
	{
		return FindCompare( Item, CompareFunc, &OutIndex );
	}

	// Requires C::operator== to be implemented
	bool	PushBackUnique( const C& Item )
	{
		uint Index;
		if( !Find( Item, Index ) )
		{
	...
	bool	PushBackUniqueCompare( const C& Item, ArrayCompareFunc CompareFunc )
	{
		uint Index;
		if( !FindCompare( Item, CompareFunc, Index ) )
		{
			PushBack( Item );
			return true;
		}
	...
	typedef bool ( *ArraySortFunc )( const C&, const C& );
	void QuickSort()
	{
		if( m_Size > 1 )
		{
	...
	void QuickSort( ArraySortFunc SortFunc )
	{
		if( m_Size > 1 )
		{
			const uint PivotIndex = m_Size / 2;
			...
			for( uint Index = 0; Index < m_Size; ++Index )
			{
				...
				{
					const C& IterValue = m_Array[ Index ];
					if( SortFunc( IterValue, PivotValue ) )	// i.e., if( IterValue < PivotValue )
					{
						Less.PushBack( IterValue );
					}
					else
			...
			Less.QuickSort( SortFunc );
			Greater.QuickSort( SortFunc );
	...
	// Insertion sort is worst case O(n^2) and requires O(1) space.
	void InsertionSort()
	{
		for( uint Index = 0; Index < m_Size; ++Index )
		{
	...
	void InsertionSort( ArraySortFunc SortFunc )
	{
		for( uint Index = 0; Index < m_Size; ++Index )
		{
			// Create a copy of the item. It's possible I don't want to do this in some cases.
			...
			uint Hole = Index;
			while( Hole > 0 && SortFunc( IterValue, m_Array[ Hole - 1 ] ) )
			{
				m_Array[ Hole ] = m_Array[ Hole - 1 ];
				Hole = Hole - 1;
			}
