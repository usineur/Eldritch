...
namespace SoLoud
{
	...
	void Soloud::setVoicePause(unsigned int aVoice, int aPause)
	{
		...
		if (mVoice[aVoice])
		{
			...
			else
			{
				...
				// DLP 24 Aug 2018: If a sound is still zero volume after it is unpaused, handle it (see also setVoiceVolume)
				if (mVoice[aVoice]->mOverallVolume < 0.01f)
				{
					// Inaudible.
					mVoice[aVoice]->mFlags |= AudioSourceInstance::INAUDIBLE;

					if (mVoice[aVoice]->mFlags & AudioSourceInstance::INAUDIBLE_KILL)
					{
						stopVoice(aVoice);
					}
				}
	...
	void Soloud::setVoiceVolume(unsigned int aVoice, float aVolume)
	{
		...
		if (mVoice[aVoice])
		{
			...
			// DLP 24 Aug 2018: These flags don't usually get set except in 3D sound, but... they should?
			if (mVoice[aVoice]->mFlags & AudioSourceInstance::PAUSED)
			{
				// I start sounds paused at zero volume, so this should be allowed
			}
			else
			{
				if (mVoice[aVoice]->mOverallVolume < 0.01f)
