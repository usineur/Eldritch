	...
	void Soloud::mixBus(float *aBuffer, unsigned int aSamples, float *aScratch, unsigned int aBus, float aSamplerate, unsigned int aChannels)
	{
		...
		for (i = 0; i < mActiveVoiceCount; i++)
		{
				...
				!(voice->mFlags & AudioSourceInstance::INAUDIBLE))
			{
				...
				{
					// DLP 13 Jan 2018: don't ramp volumes from 0 when starting a voice
					if (!voice->mCurrentChannelVolumeIsValid)
					{
						pan[k] = pand[k] = voice->mChannelVolume[k] * voice->mOverallVolume;
						pani[k] = 0.0f;
						continue;
					}
				...
				// DLP 13 Jan 2018: don't ramp volumes from 0 when starting a voice
				voice->mCurrentChannelVolumeIsValid = true;
