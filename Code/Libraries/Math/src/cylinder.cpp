...
bool Cylinder::Intersects( const Segment& s, CollisionInfo* const pInfo /*= NULL*/ ) const
{
	...
	if( Abs( a ) < SMALLER_EPSILON )
	{
		...
		if( pInfo )
		{
			const Vector Intersection	= s.m_Point1 + t * n;
			const Vector PointOnLine	= Line( m_Point1, m_Point2 - m_Point1 ).NearestPointTo( Intersection );
			const Vector Normal			= ( Intersection - PointOnLine ).GetNormalized();

			pInfo->m_Out_Collision		= true;
			pInfo->m_Out_Intersection	= Intersection;
			pInfo->m_Out_HitT			= t;
			pInfo->m_Out_Plane			= Plane( Normal, PointOnLine );
		}
		return true;
	}
