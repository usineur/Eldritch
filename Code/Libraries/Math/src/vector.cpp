...
Angles Vector::ToAngles() const
{
	...
	return Angles( Pitch, 0.0f, Yaw );
}

// To be used when we know we only want a yaw.
Angles Vector::ToAngles2D() const
{
	Vector	Normal	= GetNormalized();
	float	Yaw		= ATan2( -Normal.x, Normal.y );
	return Angles( 0.0f, 0.0f, Yaw );
}
