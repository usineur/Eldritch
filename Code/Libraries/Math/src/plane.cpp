...
{
	return ( ( ( m_Normal.Equals( p.m_Normal, EPSILON ) ) && ( Abs( m_Distance - p.m_Distance ) < EPSILON ) )
		|| ( ( m_Normal.Equals( -p.m_Normal, EPSILON ) ) && ( Abs( m_Distance + p.m_Distance ) < EPSILON ) ) );
}
...
bool Plane::Equals( const Plane& p, float Epsilon ) const
{
	return ( Abs( m_Distance - p.m_Distance ) <= Epsilon ) && m_Normal.Equals( p.m_Normal, Epsilon );
}
