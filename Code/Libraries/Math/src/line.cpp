...
bool Line::Intersects( const Plane& p ) const
{
	return ( !Equal( m_Direction.Dot( p.m_Normal ), 0.f ) ||		// If the line isn't parallel to the plane,
			 m_Point.Equals( p.ProjectPoint( m_Point ), EPSILON ) );	// or it is, but the point is on the plane, it intersects
}
...
Vector Line::NearestPointTo( const Line& l ) const
{
	const Vector	R = m_Point - l.m_Point;
	const float		A = 1.0f;	// Assuming m_Direction is normalized; else it should be dot( m_Direction, m_Direction )
	const float		B = m_Direction.Dot( l.m_Direction );
	const float		C = m_Direction.Dot( R );
	const float		E = 1.0f;	// Same assumption as A
	const float		F = l.m_Direction.Dot( R );
	const float		D = A * E - B * B;
	const float		S = ( B * F - C * E ) / D;
	//const float		T = ( A * F - B * C ) / D;
	return m_Point + S * m_Direction;
}