...
#define CHECK_ITERATING_ENTITIES do{ DEVASSERT( !m_IteratingEntities ); } while(0)
...
#define CHECK_ITERATING_SCENES do{ DEVASSERT( !m_IteratingScenes ); } while(0)
...
/*static*/ bool WBWorld::HasInstance()
{
...
void WBWorld::Render() const
{
	...
	// SRF: I can only render the entire world, not particular scenes
	BEGIN_ITERATING_ENTITIES;
	FOR_EACH_MAP( EntityIter, m_Entities, uint, WBEntity* )
	{
		WBEntity* const pEntity = EntityIter.GetValue();
...
// SRF: Not currently used (where would scene be deleted?)
...
void WBWorld::SetTime( const float Time )
{
	m_Clock->SetGameCurrentTime( Time );
}
