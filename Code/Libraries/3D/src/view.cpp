...
,	m_Bounds( Bounds )
{
	m_AspectRatio = ( Bounds.m_Right - Bounds.m_Left ) / ( Bounds.m_Bottom - Bounds.m_Top );
	UpdateInvRange();
}
...
{
	const Matrix	VP				= GetViewMatrix() * GetProjectionMatrix();
...
{
	const Matrix	InvVP			= ( GetViewMatrix() * GetProjectionMatrix() ).GetInverse();
