...
int Surface::LoadBMP( const IDataStream& Stream )
{
	...
	const int				SizeOfBase			= static_cast<int>( sizeof( SBitmapInfoHeader ) );
	const int				SizeOfExt			= static_cast<int>( sizeof( SBitmapInfoHeaderExt ) );
	int						Remaining			= BMPInfoHeader.m_Size - SizeOfBase;
	...
	if( Remaining > 0 )
	{
		...
		// And skip anything in header I still don't know about
		if( Remaining > 0 )
		{
	...
	if( NeedsSwizzle )
	{
		ASSERTDESC( Stride == Width * 4, "Can't swizzle a non-32-bit BMP!" );
		for( int Y = 0; Y < Height; ++Y )
		{
			for( int X = 0; X < Width; ++X )
			{
				byte* const pPixelB = m_pPixels + ( 4 * X ) + ( Y * Stride );
				byte* const pPixelG = m_pPixels + ( 4 * X ) + ( Y * Stride ) + 1;
				byte* const pPixelR = m_pPixels + ( 4 * X ) + ( Y * Stride ) + 2;
				byte* const pPixelA = m_pPixels + ( 4 * X ) + ( Y * Stride ) + 3;

				// HACKHACK for my current RGBA -> ARGB case!
				Swap( *pPixelB, *pPixelG );
				Swap( *pPixelG, *pPixelR );
				Swap( *pPixelR, *pPixelA );
			}
		}
	}
...
void Surface::Reset( int Width, int Height, int BitCount )
{
	...
	m_BitmapInfo.m_Header.m_BitCount				= static_cast<c_uint16>( BitCount );
