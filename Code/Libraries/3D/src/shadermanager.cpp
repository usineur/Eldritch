...
IShaderProgram* ShaderManager::GetShaderProgram( const SimpleString& VertexShaderFilename, const SimpleString& PixelShaderFilename, const uint VertexSignature )
{
	// HACKHACK to fix pixel grid offset the correct way, without breaking legacy
	// Redirect vertex shaders to remastered versions
	const SimpleString RemasterVertexShaderFilename = VertexShaderFilename.Replace( "HLSL/", "HLSL/Remaster/" );

	SShaderProgramKey Key;
	Key.m_VertexShaderHash = RemasterVertexShaderFilename;
	...
	{
		IVertexShader* const		pVertexShader		= GetVertexShader( RemasterVertexShaderFilename );
		IPixelShader* const			pPixelShader		= GetPixelShader( PixelShaderFilename );
		IVertexDeclaration* const	pVertexDeclaration	= m_Renderer->GetVertexDeclaration( VertexSignature );
