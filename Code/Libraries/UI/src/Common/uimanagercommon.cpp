...
/*virtual*/ void UIManagerCommon::InitializeFromDefinition( const SimpleString& DefinitionName )
{
	...
	// HACKHACK for Eldritch remaster
	STATICHASH( NumRmScreens );
	const uint NumRmScreens = ConfigManager::GetInt( sNumRmScreens, 0, sDefinitionName );
	for( uint RmScreenIndex = 0; RmScreenIndex < NumRmScreens; ++RmScreenIndex )
	{
		const SimpleString	RmScreenName	= ConfigManager::GetSequenceString( "RmScreen%d", RmScreenIndex, "", sDefinitionName );
		UIScreen* const		pRmScreen		= UIFactory::CreateScreen( RmScreenName );
		ASSERT( pRmScreen );

		AddScreen( RmScreenName, pRmScreen );
	}
	...
	const SUICallback&	OKCallback /*= SUICallback()*/ )
{
	...
	pOKDialog->SetParameters( PauseGame, OKString, OKDynamicString, OKEvent, OKCallback );
