...
void MeshCompiler::CompileAnimations( TiXmlElement* Arm )
{
	...
	for( TiXmlElement* Keyf = Arm->FirstChildElement( "anim" ); Keyf; Keyf = Keyf->NextSiblingElement( "anim" ) )
	{
		...
		Animation.m_AnimData.m_StartFrame = (c_uint16)KeyfAttr->IntValue();

		KeyfAttr = KeyfAttr->Next();
		const char* pName = KeyfAttr->Value();
		CleanStrncpy( Animation.m_AnimData.m_Name, pName, ANIM_NAME_LENGTH );
		Animation.m_AnimData.m_HashedName = Animation.m_AnimData.m_Name;
	...
	for( int i = 0; i < ( m_Header.m_NumAnims - 1 ); ++i )
	{
		...
		ThisAnim.m_AnimData.m_Length = NextAnim.m_AnimData.m_StartFrame - ThisAnim.m_AnimData.m_StartFrame;
	}
	...
	if( m_Header.m_NumAnims )
	{
		...
		ThisAnim.m_AnimData.m_Length = (c_uint16)( ( m_Header.m_NumFrames ) - ThisAnim.m_AnimData.m_StartFrame );	// -1 because of added T-pose
	}
}
...
{
	c_uint32 NumVerts = (c_uint32)m_Positions.Size();
...
void MeshCompiler::CalculateTangents()
{
	...
	for( c_uint32 i = 0; i < m_Header.m_NumVertices; ++i )
	{
...
void MeshCompiler::Write( const IDataStream& Stream )
{
	...
	else
	{
		...
		{
			Stream.WriteUInt16( ( c_uint16 )m_Indices[i] );
		}
	}
