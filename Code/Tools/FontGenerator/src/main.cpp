...
struct SFGProps
{
	...
	bool					m_NoCompress;
	SimpleString			m_Face;
	int						m_Leading;				// Previously called "height", this is the font metric tmHeight, not strictly the spacing on the font sheet
...
void DrawFont( Surface& LocaleSurface, SFGLocaleProps& LocaleProps, SFGProps& Props )
{
	...
	LogFont.lfQuality			= Props.m_Antialias ? ANTIALIASED_QUALITY : NONANTIALIASED_QUALITY;	// TODO LATER: Try CLEARTYPE_QUALITY (and CLEARTYPE_NATURAL_QUALITY?)
	...
	Props.m_Leading = TextMetric.tmHeight;
	...
	Props.m_VerticalC = ( ExtraHeight - Props.m_VerticalA );
...
void SaveProps( const IDataStream& Stream, const SFGProps& Props )
{
	...
	{
		Stream.WriteInt32( -1 ); // HACKHACK to signal remaster font format
		Stream.WriteInt32( Props.m_Leading );
...
void PopulateProps( SFGProps& Props )
{
	...
	STATICHASH( Weight );
	Props.m_Weight = ConfigManager::GetInt( sWeight, 400 );
	...
	// LEGACY
	STATICHASH( LegacyFormat );
	Props.m_LegacyFormat = ConfigManager::GetBool( sLegacyFormat );

	STATICHASH( CharHeight );
	const int CharHeight = ConfigManager::GetInt( sCharHeight );
	if( CharHeight > 0 )
	{
		// HACKHACK for LEGACY: this is also a signal that we're baking a legacy font
		Props.m_LegacyFormat = true;
		Props.m_Leading = Props.m_Height = ConfigManager::GetInt( sCharHeight );
	}

	STATICHASH( MinHeight );
	Props.m_Height = ConfigManager::GetInt( sMinHeight, Props.m_Height );
